...
class SolutionLines(models.Model):
    ...
    def _compute_qty(self):
        for line in self:
            line.product_qty = (line.final_product_id.size_subtotal*line.percentage)/100
...
class Productlines(models.Model):
    ...
    size_total = fields.Integer(string="Total size(gr)", compute="_compute_size_total")
    pharmaceutical_form = fields.Many2one('product.product', string="Pharmaceutical form", required=True)
    value_pharmaceutical_form = fields.Float(string="Value form pharmaceutical")

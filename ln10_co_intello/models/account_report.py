...
class AccountReport(models.AbstractModel):
    ...
    def report_is_multibook(self, model):
        ...
        report_mul_book.append(self.env["account.aged.receivable"])
        report_mul_book.append(self.env["account.aged.payable"])
...
class ReportAccountAgedReceivable(models.AbstractModel):
    ...
    @api.model
    def _get_lines(self, options, line_id=None):
        lines = super(ReportAccountAgedReceivable, self)._get_lines(options, line_id)
        for line in lines:
            line.update({
                'name': self.book_and_name(line['name'].split(" ", )[0], line['name']),
                'title_hover': self.book_and_name(line['name'].split(" ", )[0], line['name']),
...
class ReportAccountAgedPayable(models.AbstractModel):
    ...
    @api.model
    def _get_lines(self, options, line_id=None):
        lines = super(ReportAccountAgedPayable, self)._get_lines(options, line_id)
        for line in lines:
            line.update({
                'name': self.book_and_name(line['name'].split(" ", )[0], line['name']),
                'title_hover': self.book_and_name(line['name'].split(" ", )[0], line['name']),
...
#        # +120     : 2018-10-10
#
#        book_w = self.env['book.report.wizard'].search([])[-1].book_id.id
#
#        ctx = self._context
#        periods = {}
...
#            args_list += (date_from, tuple(company_ids),str(book_w))
#
#            query = '''SELECT l.id
